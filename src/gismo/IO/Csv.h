/** @file Csv.h
    ...
    @brief Provides functions for writing CSV (Comma Separated Values) files.
...
/// @brief Export a gsMatrix to a CSV (comma separated values) file
/// @tparam T numeric type of the matrix entries
/// @param filename path of output file
/// @param matrix a gsMatrix to be written to the file
/// @param headers optionally, a vector of strings to be used as column headers
///
/// \ingroup IO
template<class T>
void writeMatrixToCsv(std::string const & filename, const gsMatrix<T> & matrix, const std::vector<std::string> & headers = std::vector<std::string>() )
{
    ...
    GISMO_ASSERT( (headers.empty() ||  headers.size() == (size_t)matrix.cols()),
...
/// @deprecated Use writeMatrixToCsv instead
template<class T>
...
{
    writeMatrixToCsv(filename, matrix, headers);
}
